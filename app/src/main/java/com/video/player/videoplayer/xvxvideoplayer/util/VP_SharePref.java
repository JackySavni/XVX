...
public class VP_SharePref {
    ...
    public static boolean vp_nativeOutSide(Activity context) {
        return context.getSharedPreferences(vp_SharedPrefName, MODE_PRIVATE)
                .getBoolean("1NativeOutSide", false);
    }
    ...
    public static void vp_showInterAd(Activity activity, Intent intent) {
        ...
        }else if (vp_clickCount >= vp_click && vp_1FBEnable(activity) &&  fbInterAd1 != null && vp_adTimerPrefFB(activity)) {
            vp_clickCount = 0;
    ...
    public static void vp_showTrendInterAd(Activity vp_activity, Intent intent, boolean finish) {
        ...
        }else if (vp_clickTrendCount >= vp_click && vp_1FBEnable(vp_activity) &&  fbInterAd1 != null && vp_adTimerPrefFB(vp_activity)) {
            isInterTrendShown = true;
            vp_clickTrendCount = 0;
        ...
        if (isInterTrendShown) {
            return;
        }

        if (vp_dEnabled(vp_activity) && vp_adTimerPrefTrendQureka(vp_activity)) {
            isDInterShown = true;
        ...
        if(vp_gEnabled(vp_activity)){
            vp_showAdDialog(vp_activity);
            vp_timer = new CountDownTimer(4000, 1000) {
                public void onTick(long millisUntilFinished) {
                    if (millisUntilFinished > 1000) {
                        ...
                        showInterAdWithoutProgress(vp_activity,vp_click,vp_intent1,vp_isFinish);
                    }
                }
